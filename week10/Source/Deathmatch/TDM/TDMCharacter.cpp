...
ATDMCharacter::ATDMCharacter(const FObjectInitializer& ObjectInitializer)
	// Swap in the hardened CharacterMovementComponent so the authoritative server path
	// validates movement speed. Must be done here, at construction, via the ObjectInitializer.
	: Super(ObjectInitializer.SetDefaultSubobjectClass<UHardenedCharacterMovementComponent>(ACharacter::CharacterMovementComponentName))
{
...
void ATDMCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(ATDMCharacter, CurrentAmmo);
}

void ATDMCharacter::ConsumeAmmo()
{
...
void ATDMCharacter::RefillMagazine()
{
	if (HasAuthority())
	{
		CurrentAmmo = MagazineSize;
